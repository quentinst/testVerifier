...
public class ApkSignatureSchemeV2Verifier {
	...
	public static long ApkSignBlockFA = 0x0;
	public static StringBuilder pairsSB = new StringBuilder();
	public static StringBuilder signersSB = new StringBuilder();
	public static StringBuilder signedDataSB = new StringBuilder();
	public static StringBuilder signaturesSB = new StringBuilder();
	public static StringBuilder publicKeyBytesSB = new StringBuilder();
	
	
	public static long getByteBufferStartOffset(ByteBuffer b) {
		return ApkSignBlockFA + b.arrayOffset() + b.position();
	}
	
	public static long getByteBufferEndOffset(ByteBuffer b) {
		return ApkSignBlockFA + b.arrayOffset() + b.position() + b.limit();
	}
	
	public static long getByteBufferEndOffset(ByteBuffer b, long len) {
		return ApkSignBlockFA + b.arrayOffset() + b.position() + len;
	}
	
	public static String getPrefix(ByteBuffer b) {
		return String.format("[0x%04x-0x%04x]", getByteBufferStartOffset(b), getByteBufferEndOffset(b));
	}
	
	public static String getPrefix(ByteBuffer b, long len) {
		return String.format("[0x%04x-0x%04x]", getByteBufferStartOffset(b), getByteBufferEndOffset(b, len));
	}
	...
	public static String appendString(String s, int n) {
		...
		for (int i=0; i < items.length; i++) {
			...
			ret.append(String.format(t, "", items[i]));
		}
		return ret.toString();
	}
	
	public static String getString(byte[] msg, int prefix) {
    ...
    private static X509Certificate[][] verify(
        ...
        signersSB.append(String.format("%s%1sSigners\n", getPrefix(signers), ""));
        
        if (level > 0) {
        	Log.i("\n-----------------------------SignersSequence beigin-----------------------------\n");
        	Log.p(signers);
        	Log.i("\n-----------------------------SignersSequence end--------------------------------\n");
        }
        ...
        while (signers.hasRemaining()) {
            ...
            try {
                ...
                signersSB.append(String.format("%s%1sSigner #%d\n", getPrefix(signer), "", signerCount));
                if (level > 0) {
                	Log.i("\n-----------------------------signerBlock beigin-----------------------------\n");
                	Log.p(signer);
                	Log.i("\n-----------------------------signerBlock end--------------------------------\n");
    ...
    private static X509Certificate[] verifySigner(
        ...
        signedDataSB.append(String.format("%s signedData\n", getPrefix(signedData)));
        if (level > 0) {
        	Log.i("\n-----------------------------signedData beigin-----------------------------\n");
        	Log.p(signedData);
        	Log.i("\n-----------------------------signedData end--------------------------------\n");
        ...
        signaturesSB.append(String.format("[0x%04x-0x%04x] signatures\n",
        		ApkSignBlockFA + getByteBufferStartOffset(signatures) - 4,
        		ApkSignBlockFA + getByteBufferEndOffset(signatures)
        		));
        ...
        publicKeyBytesSB.append(String.format("[0x%04x-0x%04x] publicKeyBytes\n", 
        		getByteBufferEndOffset(signatures), getByteBufferEndOffset(signatures) + publicKeyBytes.length + 4));
        
        if (level > 0) {
        	Log.i("\n-----------------------------publicKeyBytes beigin-----------------------------\n");
        	Log.p(publicKeyBytes);
        	Log.i("\n-----------------------------publicKeyBytes end--------------------------------\n");
        }
        ...
        while (signatures.hasRemaining()) {
            ...
            try {
                ...
                //signaturesSB.append(String.format("%s%1ssignature #%d\n", getPrefix(signature), "", signatureCount));
                signaturesSB.append(String.format("[0x%04x-0x%04x] signature #%d\n",
                		ApkSignBlockFA + getByteBufferStartOffset(signature) - 4,
                		ApkSignBlockFA + getByteBufferEndOffset(signature),
                		signatureCount));
                ...
                signaturesSB.append(String.format("%16ssignature #%d sigAlgorithm=%x (%s)\n", 
                		"", signatureCount, sigAlgorithm, getSignatureAlgorithmJcaSignatureAlgorithm(sigAlgorithm).first));
                
                byte[] tmp = new byte[signature.limit()];
                System.arraycopy(signature.array(), signature.arrayOffset() + 4, tmp, 0, signature.limit());
                signaturesSB.append(getString(tmp, 20));
        ...
        signaturesSB.append(String.format("%16ssignature count=%d bestSigAlgorithm=%x keyAlgorithm=%s(%s)\n", 
        		"", signatureCount, bestSigAlgorithm, keyAlgorithm, jcaSignatureAlgorithm));
        ...
        try {
            ...
            publicKeyBytesSB.append(appendString(publicKey.toString(), 0x10));
        ...
        signedDataSB.append(String.format("%s digests\n", getPrefix(digests)));
        ...
        while (digests.hasRemaining()) {
            ...
            try {
                ...
                signedDataSB.append(String.format("[0x%04x-0x%04x] digest #%d sigAlgorithm=%x bestSigAlgorithm=%x\n", 
                		ApkSignBlockFA + getByteBufferStartOffset(digest),
                		ApkSignBlockFA + getByteBufferEndOffset(digest) - 4,
                		 digestCount, sigAlgorithm, bestSigAlgorithm));
                
                if (sigAlgorithm == bestSigAlgorithm) {
                    contentDigest = readLengthPrefixedByteArray(digest);
                    
                    signedDataSB.append(String.format("%15s digest #%d contentDigest len=%d\n", "",  digestCount, contentDigest.length));
                    signedDataSB.append(getString(contentDigest, 20));
                    
                    if (level > 0) {
                    	Log.i("\n-----------------------------contentDigest beigin-----------------------------\n");
                    	Log.p(contentDigest);
                    	Log.i("\n-----------------------------contentDigest end--------------------------------\n");
                    }
                }
        ...
        signedDataSB.append(String.format("[0x%04x-0x%04x] certificates\n",
        		ApkSignBlockFA + getByteBufferStartOffset(certificates) - 4,
        		ApkSignBlockFA + getByteBufferEndOffset(certificates) - 4 ));
        ...
        while (certificates.hasRemaining()) {
            ...
            signedDataSB.append(String.format("[0x%04x-0x%04x] certificate #%d\n", 
            		ApkSignBlockFA + getByteBufferStartOffset(certificates) - encodedCert.length - 4, 
            		ApkSignBlockFA + getByteBufferStartOffset(certificates) - 4,
            		certificateCount));
            ...
            signedDataSB.append(appendString(certificate.toString(), 16));
            if (level > 0) {
            	Log.i("\n-----------------------------encodedCert beigin-----------------------------\n");
            	Log.p(encodedCert);
            	Log.i("\n-----------------------------encodedCert end--------------------------------\n");
        ...
        printSB(signedDataSB);
        printSB(signaturesSB);
        printSB(publicKeyBytesSB);
    ...
    private static Pair<ByteBuffer, Long> findApkSigningBlock(
        ...
        Log.i(String.format("apkSigBlockOffset = 0x%x totalSize = 0x%x\n", apkSigBlockOffset, totalSize));
        Log.i(String.format("apkSigBlockSizeInHeader\t\t= 0x%x", apkSigBlockSizeInHeader));  
        Log.i(String.format("%s", "IDValuePairSequences\t\t= {...}"));
        Log.i(String.format("apkSigBlockSizeInFooter\t\t= 0x%x", apkSigBlockSizeInFooter));
        Log.i(String.format("magic\t\t\t\t= '%s'", "APK Sig Block 42"));
        Log.i(String.format("\n[*]apkSignBlock len=0x%x\n", apkSigBlock.limit()));
        if (level > 0) 
        {
        	Log.i("\n-----------------------------apkSignBlock beigin-----------------------------\n");
        	Log.p(apkSigBlock);
    ...
    private static ByteBuffer findApkSignatureSchemeV2Block(ByteBuffer apkSigningBlock)
        ...
        Log.i(String.format("%s IDValuePairSequences", getPrefix(pairs)));
        if (level > 0) {
        	Log.i("\n-----------------------------IDValuePairSequences beigin-----------------------------\n");
        	Log.p(pairs);
        	Log.i("\n-----------------------------IDValuePairSequences end--------------------------------\n");
        }
        ...
        while (pairs.hasRemaining()) {
            ...
            pairsSB.append(String.format("[0x%04x-0x%04x] Pair #%d\n", 
            		ApkSignBlockFA + pairs.arrayOffset() + pairs.position(), 
            		ApkSignBlockFA + pairs.arrayOffset() + pairs.position() + lenLong,
            		entryCount));
            ...
            int id = pairs.getInt();
            //Log.i(String.format("[0x%04x+0x%04x]%sPair #%d len=%x ID=0x%x", "", entryCount, lenLong, id));
            pairsSB.append(String.format("%16sPair #%d ID=0x%x", "", entryCount, id));
    ...
    private static final class MemoryMappedFileDataSource implements DataSource {
        ...
        private static final long MEMORY_PAGE_SIZE_BYTES = 1024;
        ...
        public void feedIntoMessageDigests(
			...
			inChannel.position(filePosition);
